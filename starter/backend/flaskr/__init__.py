...
def create_app(test_config=None):
  ...
  def play_quiz_question():
      ...
      # Assurez-vous que les clés nécessaires sont présentes dans les données reçues
      ...
      # Récupérez les questions basées sur la catégorie sélectionnée
      if quiz_category['id'] == 0:
          questions = Question.query.all()
      else:
          questions = Question.query.filter_by(category=quiz_category['id']).all()

      # Générez une question aléatoire qui n'a pas été posée précédemment
      ...
      # Essayez de trouver une question qui n'a pas été posée précédemment
      while found:
