...
class AdminloginView(APIView):
    ...
    def get(self, request, format=None):
        # Si quelqu'un tente d'accéder à cette vue via GET, nous renvoyons une réponse vide avec un statut 200 OK
...
class AdminDashboardMetricsView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            metrics_data = {
                "admin_name": admin_user.name,  # Accès au nom de l'utilisateur
...
# Vue pour le marquage de présence par l'administrateur
class AdminMarkAttendanceViews(APIView):
    ...
    def post(self, request):
        ...
        # Valider le code QR et les coordonnées géographiques
        try:
            student_email, timestamp = qr_code_data.split("_")
            timestamp = float(timestamp)
            current_timestamp = timezone.now().timestamp()
            if abs(current_timestamp - timestamp) > 60:  # Ajustez le délai selon vos besoins
        ...
        # Valider la géofence
        if not is_within_geofence(latitude, longitude):
            return Response({"error": "Device is outside from geofence location"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            Attendance.objects.create(student=student, date=date, status="Present")
            return Response({"msg": "Attendance marked successfully."}, status=status.HTTP_200_OK)
        except Student.DoesNotExist:
            return Response({"error": "Student not found."}, status=status.HTTP_404_NOT_FOUND)
from django.views.decorators.csrf import csrf_exempt
      
@method_decorator(csrf_exempt, name='dispatch')
@method_decorator(AuthenticationMiddleware, name='dispatch')
class AdminMarkAttendanceAPIView(APIView):
    """
    API view to handle marking attendance.
    """
    ...
    def get(self, request):
        ...
        serializer = AdminAttendanceSerializer(attendance_records, many=True)
        return Response(serializer.data)
    ...
    def put(self, request, pk):
        ...
        """
        attendance_instance = get_object_or_404(Attendance, pk=pk)
        serializer = AdminAttendanceSerializer(attendance_instance, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
    ...
    def post(self, request):
        qr_code_data = request.data.get("qr_code_data")
        latitude = request.data.get("latitude", "37.7749")  # Valeur par défaut pour latitude
        longitude = request.data.get("longitude", "-122.4194")  # Valeur par défaut pour longitude
